...
public class BookRecommendationsFrame(var accessToken : String?) : JFrame() {
	...
	private fun getBookRecommendations(accessToken : String?) : BookRecommendations? {
		...
		try
		{
			...
			// val type : JavaType = mapper.getTypeFactory().constructParametricType(OperationResult::class, BookRecommendations::class);
			// val type : JavaType = mapper.getTypeFactory().constructParametricType(javaClass<OperationResult>(), javaClass<String>());
			val result : OperationResult<BookRecommendations> = mapper.readValue<OperationResult<String>>(responseString,
